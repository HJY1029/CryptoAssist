...
class AESHelper:
    def __init__(self, api_key, mode="CBC"):
        ...
        self.work_dir = os.path.join(os.getcwd(), f"aes_{self.mode.lower()}_workdir")
    ...
    def _generate_c_code(self):
        """生成AES加密代码，彻底禁止使用AES_MAX_KEY_LENGTH"""
        mode_specifics = {
            "CBC": {"function": "AES_cbc_encrypt", "iv_required": True}
        }

        # 核心提示词 - 强制密钥数组定义
        base_prompt = """仅输出纯C代码，不包含任何其他内容。
必须满足：
...
3. 变量强制要求：
- 密钥：unsigned char key[32]（必须是32字节，禁止使用任何宏）
- 长度变量：全部使用size_t类型
- IV：unsigned char iv[16]
...
5. 加密函数：AES_cbc_encrypt

6. 禁止：
- 任何注释
- 任何自然语言
- 代码标记
- 使用AES_MAX_KEY_LENGTH
- 非代码内容

只输出C代码！"""

        # 错误反馈 - 针对性解决AES_MAX_KEY_LENGTH问题
        error_feedback = ""
        if self.last_error and "AES_MAX_KEY_LENGTH" in self.last_error:
            error_feedback = "必须使用unsigned char key[32]，绝对不能用AES_MAX_KEY_LENGTH，只输出纯C代码！"
        ...
        if error_feedback:
            ...
            messages.append({"role": "user", "content": "生成符合要求的AES-CBC加密代码"})
...
if __name__ == "__main__":
    # 示例使用（实际使用时应从命令行或配置文件获取API Key）
    api_key = input("请输入智谱API Key: ")
    helper = AESHelper(api_key, mode="CBC")
    helper.process()
